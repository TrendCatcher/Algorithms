...
public class boj_10866 {
    public static void main(String[]args) throws IOException {
        ...
        while (count > 0) {
            String cmd = br.readLine().trim();  //혹시 모를 공백을 제공하고
            String[] parts = cmd.split(" ");    // String 배열을 통해 cmd부분, 숫자부분을 입력받음

            switch (parts[0]) {
                case "push_front":
                    list.offerFirst(Integer.parseInt(parts[1]));
                    break;
                case "push_back":
                    list.offerLast(Integer.parseInt(parts[1]));
                    break;
                case "pop_front":
                    if(list.isEmpty()){
                        sb.append(-1).append("\n");
                ...
                case "front":
                    if(list.isEmpty()){
                        sb.append(-1).append("\n");
                    }else sb.append(list.peekFirst()).append("\n");
                    break;
                case "back":
                    if(list.isEmpty()){
                        sb.append(-1).append("\n");
